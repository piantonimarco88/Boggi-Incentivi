...
function loadAnagraficaExcel(file){
  ...
  reader.onload=function(ev){setTimeout(function(){
    try{
      ...
      // === RILEVAZIONE COLONNE INTERNAZIONALI DAL HEADER ===
      ...
      for(var ri=hdrRow+1;ri<json.length;ri++){
        var row=json[ri];if(!row)continue;
        var cdc=row[C_STORE];if(!cdc)continue;
        cdc=parseInt(cdc);if(isNaN(cdc))continue;

        var cognome=String(row[C_COGN]||"").trim();
        var nome=String(row[C_NOME]||"").trim();
        var fullName=cognome+" "+nome;
        if(!cognome&&!nome)continue;
        ...
        // Ente
        var ente=parseInt(row[C_ENTE])||210;
        ...
        // Job
        var rawJob=row[C_JOB]?String(row[C_JOB]).trim():"SA";
        ...
        // Budget
        var bdg=parseNum(String(row[C_BDG]||"0").trim());

        // Store name
        var storeDesc=String(row[C_SDESC]||"").trim();
        var storeName=(cdc+" "+(storeDesc||"Store "+cdc)).toUpperCase();

        // FC + email columns rilevate sopra dal header
        var fcColINT=C_FC;
        var emailCol=C_EMAIL;

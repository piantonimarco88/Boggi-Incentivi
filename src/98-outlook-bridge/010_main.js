...
function sendMailEmployees(){
  ...
  if(window.chrome&&window.chrome.webview){
    // Priorità: 1) CFG_PDF_PATH configurato, 2) path base memorizzato da "Salva Tutti i PDF"
    var _baseF=CFG_PDF_PATH?CFG_PDF_PATH.replace(/[\/\\]$/,""):(_pdfSaveBaseFolder||null);
    if(_baseF){
      var _psf2=getPdfSubfolder();
      var _autoFolder=_baseF+"\\"+_psf2.cons.replace(/\//g,"\\");
      genNext(0,_autoFolder);
    } else {
      // Fallback: mostra picker con suggerimento se disponibile
      ...
      _folderHandler=function(ev){
        if(typeof ev.data==='string'){
          if(ev.data.startsWith('folderSelected:')){
            ...
            // Memorizza la cartella selezionata per usi futuri
            var _psf3=getPdfSubfolder();
            var _sub=_psf3.cons.replace(/\//g,"\\");
            if(sel.endsWith("\\"+_sub)||sel.endsWith("/"+_psf3.cons)){
              _pdfSaveBaseFolder=sel.slice(0,sel.length-_sub.length-1);
            }else{
              _pdfSaveBaseFolder=sel;
            }
            genNext(0,sel);

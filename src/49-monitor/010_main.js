...
function _mBuildEnteRow(rows,year,month,ente){
  ...
  var rb=_mSum(rows,'rb_eur'),rcs=_mSum(rows,'rcs_eur');
  var fcvm=_mSum(rows,'fcvm_actual_eur'),seas=_mSum(rows,'seasonal_eur');
  var Ev=Math.round(rb+vi+rd+rp+rs+ra+rsa+rdc+rcs+fcvm+seas);
...
function _mBuildMatRow(r){
  var Ev=Math.round((r.rb_eur||0)+(r.vi_eur||0)+(r.rd_eur||0)+(r.rp_eur||0)+(r.rs_eur||0)+(r.ra_eur||0)+(r.rsa_eur||0)+(r.rdc_eur||0)+(r.rcs_eur||0)+(r.fcvm_actual_eur||0)+(r.seasonal_eur||0));
...
function renderMonitorSection(){
  ...
  if(!MONITOR_SNAPS.length){
    ...
    MONITOR_SNAPS.slice().sort(function(a,b){return a.year!==b.year?a.year-b.year:a.month!==b.month?a.month-b.month:a.created_at<b.created_at?-1:1;}).forEach(function(s){
      var tot=s.employees.reduce(function(acc,e){return acc+(e.rb_eur||0)+(e.vi_eur||0)+(e.rd_eur||0)+(e.rp_eur||0)+(e.rs_eur||0)+(e.ra_eur||0)+(e.rsa_eur||0)+(e.rdc_eur||0)+(e.rcs_eur||0)+(e.fcvm_actual_eur||0)+(e.seasonal_eur||0);},0);

...
function _buildMensileSnap(){
  ...
  E.forEach(function(e){
    ...
    var vi,rd,rp,rs,ra,rsa,rdc,rb,rcs;
    if(isUSA(e.si,e)){
      // USA: single aggregate prize (no individual KPI breakdown)
      vi=calcUSA(e)+aggTotal(e.m);
      rd=0;rp=0;rs=0;ra=0;rsa=0;rdc=0;rb=0;rcs=0;
    }else{
      // vi: calcE aggiunge getVal(e,"vi") SENZA isOn — replica comportamento esatto
      vi=getVal(e,"vi")*sm;
      ...
      // pq (QTY Dept): in IT, calcolato da calcE ma finora mai salvato nello snapshot
      var pq=isOn(e.j,"pq")?getVal(e,"pq")*sm:0;
    ...
    employees.push({
      ...
      rdc_eur:Math.round(rdc*ex*100)/100,rcs_eur:Math.round(rcs*ex*100)/100,
      pq_eur:Math.round(pq*ex*100)/100,
      fcvm_actual_eur:0,seasonal_eur:0
    });
  });
  MODE=oldMode;
...
function _mBuildEnteRow(rows,year,month,ente){
  ...
  var rb=_mSum(rows,'rb_eur'),rcs=_mSum(rows,'rcs_eur'),pq=_mSum(rows,'pq_eur');
...
function renderMonitorSection(){
  ...
  if(!MONITOR_SNAPS.length){
    ...
    MONITOR_SNAPS.slice().sort(function(a,b){return a.year!==b.year?a.year-b.year:a.month!==b.month?a.month-b.month:a.created_at<b.created_at?-1:1;}).forEach(function(s){
      var tot=s.employees.reduce(function(acc,e){return acc+(e.rb_eur||0)+(e.vi_eur||0)+(e.rd_eur||0)+(e.rp_eur||0)+(e.rs_eur||0)+(e.ra_eur||0)+(e.rsa_eur||0)+(e.rdc_eur||0)+(e.rcs_eur||0)+(e.pq_eur||0)+(e.fcvm_actual_eur||0)+(e.seasonal_eur||0);},0);

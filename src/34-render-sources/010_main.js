function rSources(){try{
  ...
  if(PRIZE_MODE==="mensile"&&!isP&&typeof demoltActive==="function"&&demoltActive())sh+='<label class="exp-btn" style="cursor:pointer;display:inline-flex;align-items:center;gap:4px;border-color:#d4a94e;color:#a07d2c" title="Store Id, Working days, Sended/Completed tasks, Accuracy — riduce il premio negozio (SM/VSM) in base al Demoltiplicatore Inventari">&#128203; Inventari + Accuracy<input type="file" accept=".xlsx,.xlsm,.xls" onchange="loadDemoltInventario(this.files[0]);this.value=\'\';" style="display:none"></label>';

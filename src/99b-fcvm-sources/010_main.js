function rSourcesFcvm(){
  ...
  if(!isP&&typeof demoltActive==="function"&&demoltActive())h+='<label class="exp-btn" style="cursor:pointer;display:inline-flex;align-items:center;gap:4px;border-color:#d4a94e;color:#a07d2c" title="Store Id, Working days, Sended/Completed tasks, Accuracy — riduce il premio d\'area del Field Coach (mai i VM) in base al Demoltiplicatore Inventari">&#128203; Inventari + Accuracy<input type="file" accept=".xlsx,.xlsm,.xls,.csv" data-fcvmsrc="fcvm_demolt" style="display:none" onchange="loadFcVmFile(this)"></label>';
...
function loadFcVmFile(input){
  ...
  if(srcId==='fcvm_demolt'){loadDemoltInventario(file);return;}
...
// Import SAS dedicato FC+VM (da Luglio 2026): file per-negozio con accettazione,
// velocità, valore SAS (EUR), riserva. Fa merge in FC_RESULTS senza toccare
// fatturato/traffico (caricati dal file Risultati).
function loadFcVmSas(file){
  var reader=new FileReader();
  reader.onload=function(ev){setTimeout(function(){try{
    var data=new Uint8Array(ev.target.result);
    var wb=XLSX.read(data,{type:'array'});
    ...
    var json=XLSX.utils.sheet_to_json(ws,{header:1,defval:null,raw:true});
    if(json.length<2){alert('Foglio vuoto.');return;}
    var hdrRow=0,hdr={sid:-1,acc:-1,vel:-1,sasv:-1,sasr:-1};
    for(var ri=0;ri<Math.min(5,json.length);ri++){
    ...
    if(hdr.acc<0&&hdr.vel<0&&hdr.sasv<0){alert('Nessuna colonna SAS riconosciuta (accettazione / velocità / valore).');return;}
    var imported=0,withVal=0;
    for(var ri2=hdrRow+1;ri2<json.length;ri2++){
      var row2=json[ri2];if(!row2)continue;var rawSid=row2[hdr.sid];if(rawSid==null)continue;
      var sid=String(parseInt(rawSid));if(sid==='NaN'||sid==='0'||isNaN(parseInt(rawSid)))continue;
      if(!FC_RESULTS[sid])FC_RESULTS[sid]={sc_eur:null,footfall_cy:null,sy_cy:null};
      if(hdr.acc>=0){var av=parseFloat(row2[hdr.acc]);if(!isNaN(av)){if(av>1)av/=100;FC_RESULTS[sid].acc=Math.max(0,Math.min(1,av));}}
      if(hdr.vel>=0){var vv=parseFloat(row2[hdr.vel]);if(!isNaN(vv)){if(vv>1)vv/=100;FC_RESULTS[sid].vel=Math.max(0,Math.min(1,vv));}}
      if(hdr.sasv>=0){var sv=parseFloat(row2[hdr.sasv]);if(!isNaN(sv)){FC_RESULTS[sid].sasv_eur=Math.round(sv*100)/100;withVal++;}}
      if(hdr.sasr>=0){var rv=parseFloat(row2[hdr.sasr]);if(!isNaN(rv))FC_RESULTS[sid].sasr_eur=Math.round(rv*100)/100;}
      imported++;
    }

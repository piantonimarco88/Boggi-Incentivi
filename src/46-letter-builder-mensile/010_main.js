...
function buildLetter(e){
  ...
  if(isOn(e.j,"pq")){sN++;var val=getVal(e,"pq"),p=val>0,vS=Math.round(val*sm);h+=secHd(sN,"QTY",p,isP);if(!isP||tg.qt){h+=ltRow("TARGET","QTY "+(tg.qt||0));if(PARAMS.kpi100<1&&(tg.qt||0)>0)h+=ltRow('Soglia ('+Math.round(PARAMS.kpi100*100)+'%)','<span style="font-size:9px;color:#a09a92">QTY '+Math.ceil((tg.qt||0)*PARAMS.kpi100)+'</span>');}h+=ltRow("MAX",'<span class="lt-row-cur">'+cu+"</span>"+Math.round(e.ib*PARAMS.qtyPct));if(!isP)h+=ltRow("RESULT","QTY "+(cn.qc||0));h+=secPd(pdLabel,vS,cu,p);h+="</div></div>"}
  ...
  if(isOn(e.j,"rp")){sN++;var val=getVal(e,"rp"),p=val>0,vS=Math.round(val*sm);var _rlP=e.rl||e.ib||0;h+=secHd(sN,tr(e,"privilege","PRIVILEGE"),p,isP);if(!isP||tg.pr){h+=ltRow("TARGET",fPct(tg.pr));if(PARAMS.kpi100<1&&(tg.pr||0)>0)h+=ltRow('Soglia ('+Math.round(PARAMS.kpi100*100)+'%)','<span style="font-size:9px;color:#a09a92">'+fPct((tg.pr||0)*PARAMS.kpi100)+'</span>');}h+=ltRow("FORMULA",(PARAMS.privPct*100)+"% RLM");h+=ltRow("INCENTIVE MAX",'<span class="lt-row-cur">'+cu+"</span>"+Math.round(_rlP*PARAMS.privPct));if(!isP)h+=ltRow("RESULT",fPct(cn.nf));h+=secPd(pdLabel,vS,cu,p);h+="</div></div>"}
  if(isOn(e.j,"rs")){sN++;var val=getVal(e,"rs"),p=val>0,vS=Math.round(val*sm);var _rlS=e.rl||e.ib||0;h+=secHd(sN,"SY TARGET",p,isP);if(!isP||tg.sy){h+=ltRow("TARGET",fDec(tg.sy));if(PARAMS.kpi100<1&&(tg.sy||0)>0)h+=ltRow('Soglia ('+Math.round(PARAMS.kpi100*100)+'%)','<span style="font-size:9px;color:#a09a92">'+fDec(Math.round((tg.sy||0)*PARAMS.kpi100*10)/10)+'</span>');}h+=ltRow("FORMULA",(PARAMS.syPct*100)+"% RLM");h+=ltRow("INCENTIVE MAX",'<span class="lt-row-cur">'+cu+"</span>"+Math.round(_rlS*PARAMS.syPct));if(!isP)h+=ltRow("RESULT",fDec(cn.sy));h+=secPd(pdLabel,vS,cu,p);h+="</div></div>"}

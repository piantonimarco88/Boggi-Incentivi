...
function sasLetterBlock(lang,info,isP,cu){
  ...
  h+='<div style="font-size:10px;color:#8a8680;line-height:1.45;margin-bottom:8px">'+esc(T.logic)+'</div>';
  ...
  h+='</tbody></table>';
  if(!isP){
    if(!hasData||info.pctMatrix==null){h+='<div style="font-size:10.5px;color:#a09a92;font-style:italic">'+esc(T.nodata)+'</div>';}
    else{
      ...
      if(info.to>0){
        var _reached=info.pct>=PARAMS.bdg100,_pctWo=info.base/info.to;

  ...
  if(MODE==="preventivo"){IT.forEach(function(it){if(it.k!=="vi"&&!isOn(e.j,it.k))return;if(it.k==="ra"&&!PARAMS.artEnabled)return;var val=getVal(e,it.k);
    ...
    IT.forEach(function(it){if(it.k!=="vi"&&!isOn(e.j,it.k))return;if(it.k==="ra"&&!PARAMS.artEnabled)return;var val=getVal(e,it.k),paid=val>0,vS=Math.round(val*sm);
      switch(it.k){
        ...
        case "rsa":if(sasZeroByAcc(e)){var saTx=(cn.sa==null?"n/d":((cn.sa*100).toFixed(0)+"%"));R.push({t:"fail",x:fc(0,cu)+" \u2014 SAS azzerato: % accettati "+saTx+" < soglia "+Math.round(PARAMS.sasMinAccPct*100)+"%."});}else if(paid)R.push({t:"success",x:fc(vS,cu)+" \u2014 "+(cn.s4||0)+" SAS"+(cn.sa!=null?" \u2014 % accettati: "+((cn.sa*100).toFixed(0)+"%"):"")+"."});else R.push({t:"fail",x:fc(0,cu)+" \u2014 SAS."});break;

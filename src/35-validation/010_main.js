...
function validateAnagrafica(){
  ...
  if(typeof RL!=="undefined"&&Array.isArray(RL)){
    ...
    Object.keys(unknownJobs).forEach(function(j){
      issues.warnings.push("Ruolo \""+j+"\" non in lista ufficiale (presente in "+unknownJobs[j]+" righe)");
    });
  }
  ...
  if(nNoSalary>0){
    issues.warnings.push(nNoSalary+" dipendenti con stipendio lordo (ib) mancante o non valido");
  }
